...
[^7]:
     For source code see Github repository (Gerlach, 2021).

[^8]:
     There are a lot of metrics in the visualisation and a PCA of intertopic distance (the Shannon-Jensen distance is used), which we due to time and space constraints cannot go into. There are references in the visualisation that explains the metrics for the interested reader.
